...
if [[ -f "${DATA_ROOT}/stage2_candidate_groups.csv" ]]; then
  DEFAULT_CANDIDATE_CSV="${DATA_ROOT}/stage2_candidate_groups.csv"
else
  DEFAULT_CANDIDATE_CSV="${DATA_ROOT}/caption/stage2_candidate_groups.csv"
fi
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_DIR}/experiments/slotmem_stage2}"
EXP_PREFIX_BASE="${EXP_PREFIX_BASE:-slotmem_stage2}"
...
LOW_NOISE_MAX_STEPS="${LOW_NOISE_MAX_STEPS:-${GLOBAL_MAX_STEPS:-1600}}"
HIGH_NOISE_MAX_STEPS="${HIGH_NOISE_MAX_STEPS:-${GLOBAL_MAX_STEPS:-600}}"
...
common_args=(
  ...
  --slotmem_memory_writer_hidden_dim "${SLOTMEM_MEMORY_WRITER_HIDDEN_DIM:-1024}"
  --slotmem_memory_writer_init_scale "${SLOTMEM_MEMORY_WRITER_INIT_SCALE:-0.1}"
  --slotmem_memory_writer_precision_tau "${SLOTMEM_MEMORY_WRITER_PRECISION_TAU:-0.3}"
  --slotmem_memory_writer_precision_scale "${SLOTMEM_MEMORY_WRITER_PRECISION_SCALE:-10.0}"
  --slotmem_memory_writer_max_delta_ratio "${SLOTMEM_MEMORY_WRITER_MAX_DELTA_RATIO:-0.0}"
  --slotmem_memory_writer_max_delta_norm "${SLOTMEM_MEMORY_WRITER_MAX_DELTA_NORM:-0.0}"
  --stage2_memory_encoder_lr "${STAGE2_MEMORY_ENCODER_LR:-1e-5}"
  --stage2_char_attn_lr "${STAGE2_CHAR_ATTN_LR:-1e-5}"
  --stage2_stage1_branch_prob "${STAGE2_STAGE1_BRANCH_PROB:-0.2}"
...
run_stage2_domain() {
  ...
  echo "=========================================="
  echo "Wan2.2 I2V SlotMem Stage2 Training"
  echo "=========================================="
  ...
  echo "STAGE1_PRELOAD_CKPT=${stage1_ckpt_path}"
  echo "STAGE2_MEMORY_ENCODER_LR=${STAGE2_MEMORY_ENCODER_LR:-1e-5}"
  echo "STAGE2_CHAR_ATTN_LR=${STAGE2_CHAR_ATTN_LR:-1e-5}"
  echo "STAGE2_STAGE1_BRANCH_PROB=${STAGE2_STAGE1_BRANCH_PROB:-0.2}"
  ...
  torchrun --nproc_per_node="${NUM_TRAIN_GPUS}" --master-port="${MASTER_PORT}" train_mem_Encoder.py \
    ...
    --pretrained_lora_path "${stage1_ckpt_path}"
}
...
for domain in "${domain_list[@]}"; do
  ...
  [[ -z "${domain}" ]] && continue
  run_stage2_domain "${domain}" "$(max_steps_for_domain "${domain}")" "$(pretrained_for_domain "${domain}")"
done
